...
// Start declaring the stages of the pipeline
  stages { 
    ...
    stage('Clone Repository to Kaniko') {
      steps {
        container('kaniko') {
          git url: 'https://github.com/PREngineer/CannaLogs', branch: 'main'
        }
      }
    }
    ...
    stage('Build and Push Docker Image to Registry') {
      steps {
        container('kaniko') {
          sh '''
            /kaniko/executor --context "`pwd`" --destination prengineer/cannalogs:latest
          '''
        }
      }
    }
      ...
      steps {
        sshagent(['RPi-SSH']) {
          script {
            ...
            sh '''
              echo 'if [ -f ~/jenkins-deployments/cannalogs-deployment.yaml ]; then\n' > script.sh
                echo 'kubectl delete -f ~/jenkins-deployments/cannalogs-deployment.yaml\n' >> script.sh
              echo 'fi' >> script.sh
      ...
      steps {
        sshagent(['RPi-SSH']) {
          script {
            // Define the code to run in the remote machine, then execute in k3s master node
            sh '''
            script="curl https://raw.githubusercontent.com/PREngineer/CannaLogs/main/cannalogs-deployment.yaml > ~/jenkins-deployments/cannalogs-deployment.yaml;
            kubectl apply -f ~/jenkins-deployments/cannalogs-deployment.yaml"
